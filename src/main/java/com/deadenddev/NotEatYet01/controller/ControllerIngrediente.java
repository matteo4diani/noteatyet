...
public class ControllerIngrediente {
	...
	public Iterable<Ingrediente> getMenu(@PathVariable long id) {
		Iterable<Ingrediente> i = dbingredienti.findByPiattoId(id);
		return i; 
	}
	...
	public Ingrediente post(@PathVariable long id, @RequestBody Ingrediente ingrediente) {
		if (dbpiatti.findById(id).isPresent()) {
			ingrediente.setPiatto(dbpiatti.findById(id).orElse(null));
